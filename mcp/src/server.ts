...
import { openPublication } from "./automation.js";
import { CredentialsError } from "./credentials.js";
import { SamuBrowserError } from "./samu-client.js";
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      `국립중앙도서관 자동 로그인 후 지정한 매체를 samu-webbrowser에서 엽니다.\n` +
      `사용 가능한 매체 (id | 한글명):\n` +
...
export async function handleCallTool(req: CallToolRequest): Promise<CallToolResult> {
  ...
  if (toolName === "open_publication") {
    ...
    try {
      ...
      if (e instanceof SamuBrowserError) return errorResult(e.message);
      const m = e instanceof Error ? e.message : String(e);
      return errorResult(`예상치 못한 오류: ${m}`);
    }
  }
...
if (isDirectRun) {
  main().catch((err) => {
    console.error("library-via-mcp server failed:", err);
    process.exit(1);
  });
}
